...
///----------------------------------
	...
	struct Node
	{
		Node(Mesh* mesh = nullptr) : m_mesh(mesh), m_parentName(""), m_childNode({}), m_parentNode(nullptr) {};
		...
		Matrix m_nodeTransform;

		Node* m_parentNode;
		std::vector<Node*> m_childNode;

		Animation* m_animation = nullptr;
		Bone* m_myBone = nullptr;

		bool isNegativeScaleRoot = false;
		bool isNegativeScaleNotRoot = false;
		bool isNegativeScale = false;

		void Transform(Vector3 distance, float angle, Vector3 value);
		void SetNode(Matrix parentMat, Matrix meshTM = Matrix::Identity);

		void Translate(Vector3 distance, Matrix meshTM);
		void Rotate(Quaternion nextRotation);
